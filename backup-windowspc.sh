...
#
# v1.1 11/29/2011
#   Modifications based on new server configuration
#   Also add check to verify BACKUP_BASE_DIR exists before backup is run
#
# v1.2 12/11/2011
#   Clean up formatting
...
#
# v1.3 12/23/2012
#	Switch to different approach to sending email results
#=======================================

#=======================================
# VARIABLES
#=======================================
MACHINE_NAME="windows-pc-name"
SOURCE_BACKUP_DIR="source-dir-name"
SOURCE_EXCLUDE_FILE="/opt/backups/scripts/exclude-files-windowspc"
RSYNC_OPTIONS="--archive --hard-links --human-readable --inplace --numeric-ids --delete --delete-excluded --verbose --itemize-changes"
TIMEDATE=`eval date +%Y%m%d-%H%M`

# The base directory where all backups are stored
BACKUP_BASE_DIR="/media/backups"

# The source directory for this backup
SOURCE_DIR="/media/"$MACHINE_NAME"/"$SOURCE_BACKUP_DIR
...
EMAIL_FILE=$BACKUP_DIR"/_update.email"
...
function writeHeader {
	...
	echo "    Source Directory:       $SOURCE_DIR"                              | tee -a $LOG_FILE $ERR_LOG_FILE
...
function getLastDirectory {
	...
	# First, check if LATEST even exists
...
mkdir -p $BACKUP_STAGING_DIR                                                    >>$LOG_FILE 2>>$ERR_LOG_FILE

# Run rsync command
echo "rsync $RSYNC_OPTIONS --exclude-from=$SOURCE_EXCLUDE_FILE $LINKDEST $SOURCE_DIR $BACKUP_STAGING_DIR"           | tee -a $LOG_FILE $ERR_LOG_FILE
rsync $RSYNC_OPTIONS --exclude-from=$SOURCE_EXCLUDE_FILE $LINKDEST $SOURCE_DIR $BACKUP_STAGING_DIR                  >>$LOG_FILE 2>>$ERR_LOG_FILE
RSYNC_RETURN_CODE=$?
...
if [ "$RSYNC_RETURN_CODE" = "0" ]; then
	...
	# Write this build name to the LATEST file
